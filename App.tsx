...
const App: React.FC = () => {
  ...
  const isFirstLoad = useRef(true); // Track first load to prevent alert blast
  const deletedAlertIds = useRef<Set<string>>(new Set()); // Track deleted alerts to prevent flicker
    ...
    const fetchAlerts = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Filter out alerts that were locally deleted recently to prevent flicker
          const validAlerts = alerts.filter(a => !deletedAlertIds.current.has(a.id));
          setNearbyAlerts(validAlerts);

          // On first load, ignore existing alerts to prevent immediate siren spam
          // and simply mark them as processed.
          if (isFirstLoad.current) {
            alerts.forEach(a => processedAlertIds.current.add(a.id));
            isFirstLoad.current = false;
            // Stop siren if there are no alerts or if we just loaded with alerts we want to ignore silently
          ...
          validAlerts.forEach(alert => {
            // Check if alert is new and NOT generated by self.
            // Explicitly filter out our own alert ID to prevent self-notification
            if (myRemoteAlertId && alert.id === myRemoteAlertId) return;
  ...
  // SOS Trigger Logic
  const triggerSOS = useCallback(async (type: EmergencyType = EmergencyType.GENERAL, fromStealth: boolean = false) => {
    setIsSOSActive(true);
    setEmergencyType(type);
    
    // If triggered from stealth (gesture), we KEEP the stealth overlay (screen black).
    // Otherwise (UI button), we exit stealth mode to show the SOS screen.
    if (!fromStealth) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Add to processed immediately so next poll doesn't trigger notification for self
        processedAlertIds.current.add(data.id);
      }
    } catch (e) {
  ...
  const cancelSOS = async () => {
    ...
    if (myRemoteAlertId) {
      const idToDelete = myRemoteAlertId;
      
      // OPTIMISTIC UPDATE: 
      // 1. Mark as deleted in ref to ignore it in polling
      deletedAlertIds.current.add(idToDelete);
      // 2. Remove visually from radar immediately
      setNearbyAlerts(prev => prev.filter(a => a.id !== idToDelete));
      ...
      try {
        ...
        // Clean up from ignored list after 15 seconds
        setTimeout(() => deletedAlertIds.current.delete(idToDelete), 15000);
      } catch (e) {
  ...
  if (stealthMode) {
    return (
      <StealthOverlay 
        ...
        onSOS={() => triggerSOS(EmergencyType.GENERAL, true)}
